...
class Model(nn.Module):
    ...
    def forward_MM(self, adj, image_adj, text_adj, audio_adj=None):
        ...
        if audio_adj == None:
            return embeds[:args.user], embeds[args.user:], embeds1[:args.user], embeds1[args.user:], embeds2[:args.user], embeds2[args.user:]
        else:
            return embeds[:args.user], embeds[args.user:], embeds1[:args.user], embeds1[args.user:], embeds2[:args.user], embeds2[args.user:], embeds3[:args.user], embeds3[args.user:]
    ...
    def forward_cl_MM(self, adj, image_adj, text_adj, audio_adj=None):
        ...
        if audio_adj != None:
            embeds, embedsImageAdj, embedsTextAdj, embedsAudioAdj = self.main_view_process(adj, image_adj, text_adj, image_feats,text_feats, audio_feats=audio_feats, audio_adj=audio_adj)
        else:
            embeds, embedsImageAdj, embedsTextAdj = self.main_view_process(adj, image_adj, text_adj, image_feats,text_feats)
    ...
    def main_view_process(self,adj, image_adj, text_adj,image_feats,text_feats,audio_feats=None, audio_adj=None):
        weight = self.softmax(self.modal_weight)
        embedsImageAdj = torch.concat([self.uEmbeds, self.iEmbeds])
        embedsImageAdj = torch.spmm(image_adj, embedsImageAdj)
        ...
        embeds = embeds + args.ris_lambda * F.normalize(embedsModal)
        if audio_adj != None:
            return embeds,embedsImage,embedsText,embedsAudio
        else:
            return embeds,embedsImage,embedsText
...
class Denoise(nn.Module):
    ...
    def get_log_prob(self, x, timesteps, actions):
        pred = self.forward(x, timesteps, mess_dropout=False)
        log_probs = -nn.CrossEntropyLoss(reduction='none')(pred, actions.argmax(dim=-1))
        return log_probs
...
class GaussianDiffusion(nn.Module):
    ...
    def training_losses(self, model, x_start, itmEmbeds, batch_index, model_feats, cross_feats=None):
        ...
        if cross_feats is not None:
            cross_feat = torch.mean(torch.stack(cross_feats), dim=0)
            usr_model_embeds = torch.mm(model_output, model_feats + args.cross_feat_weight * cross_feat)
        else:
